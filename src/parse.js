...
function parse_obj(txt) {
    ...
    txt.split('\n').forEach(line => {
        ...
        switch (words[0])
        {
            ...
            case 'g': {
                ...
                let feature_class = words[1].split('_')[0];
    ...
    let btriangles32 = new Float32Array(output.boundaries.triangles.flat(1))
    let deltas32 = new Float32Array(output.boundaries.deltas.flat(1))
...
function generate_class_color() {
    ...
    let class_color_dt = {}
    for (let key in class_color_raw_dt) {
        let rgb = class_color_raw_dt[key];  //rgb is a list of elements r, g, b
        let colordt = {}
        colordt.r = rgb[0];
        colordt.g = rgb[1];
        colordt.b = rgb[2];
